---
typ: sub-projekt
name: "Familie"
aliase: ["Familie", "Family"]
ueber_projekt: "[[persönlich]]"
bereich: persoenlich
umfang: offen
status: aktiv
kapazitaets_last: niedrig
kontakte: ["[[sinan-oezbek]]"]
tech_stack: []
erstellt: 2026-04-21
notizen: "Sammel-Bucket für Familien-Themen: Geschwister, Eltern, Familien-Events. Keine Family-Konflikt-Mediation (out of scope, siehe persönlich)."
quelle: user-angabe
vertrauen: extrahiert
---

## Kontext

Sub-Projekt unter [[persönlich]] für alles Familien-bezogene. Enthält Kontakte, Events, Unterstützungs-Themen (Schule, Projekte der Geschwister etc.).

## Familienmitglieder

- [[sinan-oezbek]] (Bruder, 15, 9e Bach-Gymnasium Mannheim)

## Aktueller Stand

Stand 2026-04-21:
- Sinan hält Präsentation im Fach Wirtschaft zum Thema "Künstliche Intelligenz, Bedrohung der Arbeitsplätze", Breakdown in [[sinan-ki-praesentation]]. Kritisches Feedback folgt separat.

## Offene Aufgaben

- [ ] Kritisches Feedback zu Sinans KI-Präsentation geben (Rechtschreibung, Inhalt, Logik) #mittel

## Out of Scope

- Family-Konflikt-Mediation (siehe [[persönlich]])
