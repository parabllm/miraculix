...
Technische Spec der 3 PulseBot n8n Workflows (Slack-basiertes Batch-Management). Stand 2026-04-09, produktiv im Einsatz.

## Architektur

- **Platform:** n8n Cloud
- **Interface:** Slack Slash-Command `/pulse` mit Subcommands
- **Datenspeicher:** Google Sheet (kalani drive) = SSOT mit 17 Spalten
...
└── Parse Payload (Code) - extrahiert type, callback_id, payload
    └── Switch (Route Action: {type}_{callback_id})
        ...
        │                            → Append row in sheet → Send message (Slack confirm)
        ...
        │                            → Update Received → Build Receive Response → Send Response
        ├── block_actions_tracking → Prepare Tracking Updates → Update row → Ask Confirm Tracking
        ...
        │                            → Build Lab Updates → Update Lab Status → Send message1
        └── Unknown                → Unknown Command
```
...
## WF3: Janoshik Backfill

Einmaliger Flow zur Nachverarbeitung von 32 historischen Janoshik-Test-Links. Details siehe [[janoshik-ocr]].
