...
Guide fuer Lars zum Schreiben und Verwalten von Cora's Knowledge-Chunks. Single Source of Truth ist Obsidian, die Supabase-Tabelle `knowledge_chunks` wird aus Obsidian gesynct.

## Warum Knowledge-Chunks

Cora hat kein eingebautes Fitness-Wissen. Alles was sie ueber Training, Recovery oder Ernaehrung weiss, kommt aus Knowledge-Chunks die Lars schreibt. Pro Anfrage zieht der Context-Builder die passenden Chunks aus der DB, packt sie in den Prompt, dann antwortet Gemini.
...
4. Cora benutzt den Chunk ab dem naechsten Request
...
| `author` | ja | `lars` |
| `language` | ja | `de` |
| `version` | ja | Integer. Start bei `1`. Bei groesserer Aenderung um eins hoch. |
...
| `updated_at` | optional | Datum ISO der letzten groesseren Aenderung |
...
Deutsch. Sachlich, praezise, ohne Ich-Form. Cora zitiert den Chunk nicht woertlich, aber der Ton faerbt ab. Wenn Lars "du musst" schreibt, kommt Cora darauf zurueck.
...
- Absolute Grenzwerte fuer Einzelpersonen ("bei unter X Gramm Protein droht Muskelverlust")
...
- Zahlenraeume statt Punktwerten ("1.6 bis 2.2g Protein pro kg Koerpergewicht fuer Hypertrophie")
- Wenn-Dann-Logik ("Bei Stagnation ueber 3 Wochen ist Variation sinnvoller als Volumen-Erhoehung")
...
Wenn Lars eine konkrete Quelle zitiert (Paper, Buch, Studie), als letzte Zeile auffuehren:
...
Aktuelle Werte in der DB (wahrscheinlich zu aendern):
...
Nach Positionierungsentscheidung wird das konsolidiert. Bis dahin: Lars schreibt einfach `use_cases: ["general"]` und Deniz mappt spaeter.
...
| 6 bis 7 | Wichtig fuer Teilgruppen. Beispiel: Nutrition Timing fuer Kraftsportler. |
| 4 bis 5 | Spezifisch, nur bei passendem Kontext. Beispiel: Deload-Week Indikatoren. |
| 1 bis 3 | Rand-Wissen, Bonus. |

Cora pullt bei jedem Request nur Top-N Chunks nach priority, gefiltert nach `use_cases` und `tags`. Hohe Prio = hoehere Wahrscheinlichkeit im Prompt.
...
2. Deniz liest drueber, checkt auf SaMD-Verstoesse (Diagnose-Sprache, absolute medizinische Aussagen)
...
- Knowledge-Base-Groesse: Ziel 20 bis 30 Chunks fuer v1? Mehr? Muss mit Lars abgestimmt werden
- Embedding: Die Tabelle hat keine Vector-Embeddings aktuell, Retrieval laeuft ueber `use_cases` und `priority`. Wenn Chunks zu viele werden, muessen wir pgvector dranhaengen (MVP-Grenze laut Architektur: etwa 50 Chunks).
- Chunk-Versionierung bei Aenderungen: aktuell wird alte Version ueberschrieben. Wenn wir History brauchen, muessen wir ein `knowledge_chunks_history` Table bauen.
