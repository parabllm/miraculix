---
typ: aufgabe
name: "Coralate Tech Stack & Conventions"
projekt: "[[coralate]]"
status: erledigt
...
Tech Stack, Versions, Conventions, NOT-Installed-Liste, Auth-Status, File Structure, Store Persistence Rules, Implementation Methodology, Mandatory Rules für alle künftigen Implementierungen im corelate-v3 Codebase. Stand 2026-04-09.
...
- `logged_at_unix` uses noon für past entries, `Date.now()` für today
- `deriveFoodDataState()` accepts `selectedDate` param
- Food streak pill uses food data, not workout data
- Food mutations sync zu Supabase (removed `__DEV__` guard)
...
- Always use `git pull origin main` until tracking branch set
- `git stash` before pulling when local changes exist
...
- **MANDATORY:** Jede neue full-screen page MUSS Stack screen in `app/` sein opened via `router.push()`
