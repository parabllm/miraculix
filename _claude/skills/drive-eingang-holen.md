...
- Drive-Ordner-Struktur:
  - `Miraculix_Eingang/` (Top-Level, für `.md`-Files aus Claude Mobile)
  - `Miraculix_Eingang/Images/` (Unterordner, für Binaries/Fotos die Deniz direkt via Drive-App hochlaedt)

## Wie der Skill ausgefuehrt wird

### Fall A: Claude Code im Vault (Bash verfuegbar)

1. Pruefe ob `rclone` im PATH oder über Winget-Pfad aufrufbar ist.
...
Pull ist **nicht** möglich, weil kein Zugriff auf Deniz' lokales Filesystem. Antwort:
...
- **Remote `gdrive` nicht konfiguriert:** Anweisung geben `rclone config` auszuführen, oder Deniz fragen ob die Config aus Versehen geloescht wurde (`%APPDATA%\rclone\rclone.conf`).
...
- **Ordner `Images/` nicht löschen.** Nur Inhalt movern.
...
Der Skill deckt nur die **Pull-Seite** ab. Die **Schreib-Seite** (Claude Mobile legt Notizen in `Miraculix_Eingang/`) wird über **globale Custom Instructions in claude.ai** geregelt oder über einen später als Anthropic-Skill registrierten Eintrag. Siehe separate Doku im Vault.

Namensschema für Files die Mobile in die Drive-Inbox legt:
```
