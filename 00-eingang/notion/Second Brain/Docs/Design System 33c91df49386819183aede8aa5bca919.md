...
Lifecycle: Active
Notes: Vollständiges Design System: Colors, Typography, Spacing, Motion, Haptics, Buttons, Border Radius (inkl. forbidden values), Bottom Sheet Pattern, Nutrient Colors, Glass UI. Stable weil Design Tokens gelocked sind. Source of Truth bleibt theme/theme.ts + theme/typography.ts + theme/motion.ts.
...
Vollständiges Design System für coralate: Color Tokens, Typography, Spacing, Animation/Spring Physics, Haptic Feedback Map, Button System, Border Radius System, Bottom Sheet Pattern, Status Token Hierarchy, Nutrient Accent Colors. Quelle: `theme/theme.ts`, `theme/typography.ts`, `theme/motion.ts` + Design System Doc im Cora HQ.

## Architecture / Constitution

- **Source of Truth:** `theme/theme.ts`, `theme/typography.ts`, `theme/motion.ts`
- **Do NOT use hardcoded values anywhere.** Always reference these tokens.
- **Zero emoji policy** — Icon-Library exklusiv
- **Zero `TouchableOpacity`** — always use `Pressable`

## Color Philosophy: Earned Color

Color is **earned by data**, not painted on structure. Structural elements (kickers, labels, borders, icons at rest) stay neutral. Color appears when data earns it — progress filling up, a mood selected, a score calculated.
...
- Kickers stay `textSecondary` except Cora AI (`brand.electric`)
- State-based saturation on all ring/progress indicators
...
| `brand.accent` | `#FF6B00` | Orange — **warning/caution only** (narrowed scope from v2) |
...
| `status.destructive` | `#D84343` | Standard destructive (remove set, delete food item) |
| `status.completedWorkout` | `#1A8C5B` | Completed workouts, logged meals (history checkmarks) |
...
| `status.vitalityDim` | `#007A65` | Dimmed vitality for backgrounds, subtle health cues |
| `status.amber` | `#FFD166` | Goals, progress, streaks, achievement milestones |
| `status.amberDim` | `#B38B1B` | Dimmed amber for backgrounds, subtle progress cues |
| `status.info` | `#333DDB` | AI suggestions (legacy — prefer `brand.electric` for new Cora UI) |

**`status.success` is deprecated.** New code must never reference it.
...
| `dark.surface4` | `#2E3554` | Deep floating: popovers, elevated modals, tooltips |
| `dark.textPrimary` | `#FFFBEE` | Primary text (cream) |
| `dark.textSecondary` | `#8A91A8` | Secondary text |
| `dark.textMid` | `#6B738F` | Mid-emphasis: between secondary & tertiary |
...
- All numeric data: always `fontVariant: ['tabular-nums']`
...
- **ScrollView top padding:** `LIVING_HEADER_TOP_INSET = 84` (formula: `HEADER_HEIGHT (60) + shieldBuffer (24)`)
- Exception: `DAY_STRIP_HORIZONTAL_INSET = 12` (Food screen day strip only)

## Animation / Spring Physics

Source: `theme/motion.ts` — canonical values.
...
- **NIE `borderWidth` auf buttons.** Keine outlined, ghost, oder transparent-background buttons.
- Pressed state: `opacity: 0.75` via `Pressable` callback
- Action button height: `64pt` with `borderRadius: 24`
- **Keine outlined/ghost buttons oder pills** — all interactive elements must have solid surface fill
...
Alle bottom sheets nutzen die gleiche Two-Component-Struktur: sheet wrapper (lifecycle + native presentation) und inner content component (UI).
...
| State | Use this token | Value |
| --- | --- | --- |
| Primary action confirmed | `brand.primary` | `#0000FF` |
| Workout completed / checkmarks | `status.completedWorkout` | `#1A8C5B` |
| Warning / rest day / auto-saved | `brand.accent` | `#FF6B00` |
| Destructive / delete | `status.destructive` | `#D84343` |
| Cora AI / correlation highlights | `brand.electric` | `#7B61FF` |
| Live health / vitality data | `status.vitality` | `#00C2A0` |
...
## DayStatusButton Pill Fills — Canonical Mapping
...
## Glass UI — Header Pill Elements (iOS 26+ with Fallback)

Calendar toggle icon + profile avatar in `LivingPageHeader` werden in ein shared `GlassContainer` gewrapped als pill-shaped glass cluster auf iOS 26.
...
- Check `AccessibilityInfo.isReduceTransparencyEnabled()` and skip glass if reduced transparency is enabled